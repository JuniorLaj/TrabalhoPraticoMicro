...
void main(void) {
    unsigned char modoOperacao = 0; // 0 -> sem opera��o 1 -> Login 2 -> Cadastro
    unsigned char tecla;
    ...
    // LINHA 0 DA EEPROM utilizada para cabe�alho
    unsigned char countRecordedAccesses = eeprom_read(0);
    
    // LIMPANDO TODAS AS POSI��ES N�O UTILIZADS DA EEPROM PARA GARANTIR CONTE�DO
    unsigned char i;
    ...
    /* TO DO:
        LOGIN (Jos�)
           * Testar implementa��o feita para Q4
           * Timer 0 ap�s exibi��o da mensagem de sucesso/falha no login
           * Implementar conclus�o ap�s terminar o LOGIN (setar bits, etc..)
           * Implementar Exibi��o dos textos grandes em loop
        CADASTRO (Renan)
           * Implementar entrada dos dados
           * Incrementar contador de linha utilizada da mem�ria (countRecordedAccesses) e gravar na EEPROM o novo valor
           * Setar bit de contagem de tempo (initTimerDisplayMessageQ3) 
           * Implementar conclus�o ap�s terminar o CADASTRO (setar bits, etc..)
        EXTRAS 
    ...
    */
    ...
    while (1) {
        ...
        //Q3
        //alternar digita��o de ID para PIN e Verificar cadastro na EEPROM
        if(tecla == 35 && modoOperacao == 2) {
        //Alternar digita��o de ID para PIN
            if(infoDigitada == 1){
                lcd_cmd(L_L2);
                lcd_cmd(L_CLR);
                infoDigitada = 2;
                ...
                // Setando ox00 no restante dos vetores de simbolos para garntir conte�do
                unsigned char aux = 0;
                ...
                // Q4 - Iniciando login
                unsigned char i = 0; // bit de detec��o de acesso correto
